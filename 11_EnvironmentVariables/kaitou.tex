...
\cfoot{}
\chead{\textgt{システムプログラミング２　課題４解答}}
%----------------------------------------------------------------------
\begin{document}
...
\lstinputlisting[caption=ファイルの削除(myrm.c)]{myrm.c}
...
\newpage
\lstinputlisting[caption=ディレクトリの作成(mymkdir.c)]{mymkdir.c}
...
\newpage
\lstinputlisting[caption=ディレクトリの削除(myrmdir.c)]{myrmdir.c}
...
\newpage
\lstinputlisting[caption=ハードリンクの作成(myln.c)]{myln.c}
...
\newpage
\lstinputlisting[caption=シンボリックリンクの作成(mylns.c)]{mylns.c}
...
\newpage
\lstinputlisting[caption=ファイルの移動（名前変更）(mymv.c)]{mymv.c}
...
\newpage
\lstinputlisting[caption=ファイルのモード変更(mychmod.c)]{mychmod.c}
